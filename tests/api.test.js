...
test('player discovery uses global server scope and returns every registered player', async function () {
  const ctx = apiContext(function (cmd) {
    ...
    return {
      players_loop: [
        { playerid: 'aa:aa', name: 'Apple Squeezer Intel', connected: 1, power: 1 },
        { playerid: 'bb:bb', name: 'Squeezelite', connected: 1, power: 0 }
      ]
    };
  });

  const players = await ctx.api.players('aa:aa');

  assert.equal(ctx.requests[0][0], '', 'serverstatus must not be scoped to the active player');
  assert.deepEqual(plain(players), [
    { id: 'aa:aa', name: 'Apple Squeezer Intel', connected: true, power: true },
    { id: 'bb:bb', name: 'Squeezelite', connected: true, power: false }
  ]);
});
...
test('Apple Squeezer discovery is read-only when only standard Squeezelite is installed', async function () {
  const ctx = apiContext(function (cmd) {
    if (cmd[0] === 'can') return { _can: 0 };
    throw new Error('discovery must not send an Apple Squeezer command');
  });

  const state = await ctx.api.appleSqueezerStatus('standard-squeezelite');
  assert.equal(state.available, 0);
  assert.equal(state.lifecycle, 'not-installed');
  assert.deepEqual(ctx.calls.map(function (cmd) { return cmd.slice(0, 3); }), [
    ['can', 'applesqueezer', 'status'],
    ['can', 'apple_squeezer', 'status']
  ]);
});
...
test('Ratings Light usa os dispatches server-scoped publicados para ler e gravar', async function () {
  ...
  assert.deepEqual(ctx.calls[0], ['ratingslight', 'setratingpercentnoclient', 42, 70, 0]);
  const rating = await ctx.api.getRating('p1', 42, 'ratingslight');
  assert.deepEqual(ctx.calls[1], ['ratingslight', 'getrating', 42]);
...
test('SACDPlayer API keeps probe, status, stats, prepare and evict wire shapes in api.js', async function () {
  const ctx = apiContext(function (cmd) {
    if (cmd[0] === 'can') return { _can: 1 };
    if (cmd[1] === 'cachestats') return { usage_bytes: 10, cap_bytes: 20, free_bytes: 30, binary: 1, albums_loop: [] };
    if (cmd[1] === 'status') return { success: 1, tracks_loop: [{ number: 1, state: 'ready', bytes: 9 }] };
    return { success: 1 };
  });
  assert.equal(await ctx.api.sacdPlayerAvailable(true), true);
  assert.equal((await ctx.api.sacdCacheStats()).usageBytes, 10);
  assert.equal((await ctx.api.sacdAlbumStatus('file:///A.iso#2ch-01')).tracks[0].state, 'ready');
  await ctx.api.sacdPrepareAlbum('file:///A.iso#2ch-01');
  await ctx.api.sacdEvictAlbum('0123456789abcdef/2ch');
  assert.deepEqual(plain(ctx.calls.slice(-5)), [
    ['can','sacdplayer','cachestats','?'], ['sacdplayer','cachestats'],
    ['sacdplayer','status','file:///A.iso#2ch-01'], ['sacdplayer','prepare','file:///A.iso#2ch-01'],
    ['sacdplayer','evict','0123456789abcdef/2ch']
  ]);
});
...
test('album track pages retain server totals and contributor roles without splitting a single comma name', async function () {
  const ctx = apiContext(() => ({count: 500, titles_loop: [{id:1,title:'T',composer:'Surname, Name',composer_ids:'7',albumartist:'A, B',albumartist_ids:'8,9'}]}));
  const tracks = await ctx.api.tracks('', 3, 0, 500);
  assert.equal(tracks.total, 500);
  assert.equal(tracks.sourceCount, 1);
  const composer = tracks[0].credits.find(c => c.roleId === 2);
  assert.equal(composer.name, 'Surname, Name');
  assert.deepEqual(plain(composer.ids), ['7']);
  assert.equal(tracks[0].credits.filter(c => c.roleId === 5).length, 2);
});
test('role-scoped album queries retain role and library constraints for multiple contributor ids', async function () {
  const ctx = apiContext(() => ({count:0,albums_loop:[]}));
  ctx.api.setLibrary('root');
