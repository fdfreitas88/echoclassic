...
step "3/6  referencias entre modulos"
node tools/check-modules.js || fail=1

step "4/6  contraste WCAG"
python3 tools/check-contrast.py || fail=1

step "5/6  strings.txt"
node tools/check-strings.js || fail=1

step "6/6  visible strings use the runtime dictionary"
node tools/check-visible-strings.js || fail=1
