...
- fb4a1b1 · SACDPlayer cache status, album prepare/evict actions, ready markers, Settings cache inventory, and absent-plugin degradation · [measured]
- (uncommitted) chrome audit round 4: eight UI fixes (navbar, album rows, summary, search, player volume row, OPML, queue, SACD cache) · npm test 661/661, validate 6/6 [measured] · deploy pending Felipe
- (uncommitted) collection dashboard: api year tag + folder resolution, dashboard layout record, summary figures + decades + drill kinds, iOS 9 nav + card grid + edit mode, inspector + toolbar + action sheet + playlist modal, 32 strings · npm test 690/690, validate 6/6, check-version 3.5.6, language checks clean [measured] · live acceptance and deploy pending Felipe [unverified]
- (uncommitted) collection dashboard deployed to musicplayer.local twice (restart, then JS/CSS follow-up: duration tag, No Genre as untagged, cross-root folder fallback, value column width) · npm test 691/691, validate 6/6 [measured] · live checklist 7/7 in Chrome [live] · test playlist deleted, one Play Next insert left in the queue
